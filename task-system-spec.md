...
- **Frontmatter is the machine-owned region; the body is user-owned but CLI-editable.**
  *Automated* sync writes (`status`, `create`, `move`, `archive`) touch only frontmatter
  and the board — they never rewrite the body. *Explicit user-invoked* section edits
  (`set`, `gates`) are the sanctioned exception: they rewrite one section body or flip one
  gate checkbox, and they stay line-surgical — every byte outside the target section or
  checkbox survives. The body is still never re-rendered from a model; edits splice the
  target lines and write the rest back verbatim.
...
1. **Start** → `duty get next --claim` hands you the first actionable task and marks it
   `in-progress` in one call (or `duty status <id> in-progress` for a task by id).
2. **Blocked** (missing input, failed dependency, a decision the task doesn't pre-make) →
   `duty report <id> --status blocked` pipes a report naming *exactly* what's missing and
   flips the status in one locked write, then stop. Never guess past a blocker.
3. **Working** → tick gate checkboxes as they pass (`duty gates check <id> <n>`, `--all`
   once they all pass, or edit the box by hand).
4. **Done** (all gates ticked) → `duty report <id> --status done` pipes a report — files
   changed, gate output tails, deviations (with why), follow-ups deliberately left — and
   flips the status to `done` in the same write.
...
lock (§10). A recurring agent sequence that would take the lock twice earns a one-shot
form: `report --status` ends a task (report + status flip) in one write, `gates check
--all` ticks every gate in one, `gates add` appends several at once — one intent, one
lock, one atomic write. Exit codes: `0` ok, `≠0` error with a
one-line stderr message; `2` for a missing or unknown command.

**Help & discovery.** `duty --help` groups the tree — `Author` (`init`, `create`, `set`),
`Work` (`status`, `report`, `gates`, `move`, `archive`, `delete`), `Read` (`get`),
`Interface` (`tui`, `completion`) — and opens with the five-step lifecycle (`get next` → `status
...
| `duty report <id> [--status S] [--force]` | Append stdin under `## Report` (heading created once, content accumulates). With `--status S` also flip the status (frontmatter line + board row) in the *same* locked write — the atomic done/blocked lifecycle endings; both edits are computed before either file is written, so any error lands neither the report nor the status. `--status in-progress` obeys the claim guard (`--force` to take over a live claim). Refuse empty stdin. |
...
- **Tree-wide write lock, nothing finer** — every mutating command (`create`, `status`,
  `report`, `set`, `gates`, `move`, `archive`, `delete`; not `init`, not reads) holds one advisory
