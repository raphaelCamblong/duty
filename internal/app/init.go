...
// readme is the agent-facing convention page Init drops next to the root
// board index (spec §9): the command table, the lifecycle→command mapping,
...
func (a App) Init(cwd, title string) error {
	...
	return a.fs.WriteFile(filepath.Join(dir, names.ReadmeFile), []byte(readme))
}
